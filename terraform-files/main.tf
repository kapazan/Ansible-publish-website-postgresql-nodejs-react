...
resource "aws_instance" "nodes" {
  ami = var.myami
  instance_type = element(var.instancetype, count.index)   # would it work if we say num=2
  count = var.num
  key_name = var.mykey
  vpc_security_group_ids = [aws_security_group.tf-sec-gr.id]
  tags = {
    Name = "ansible_${element(var.tags, count.index )}" # than here there should be 2 tags i guess
    stack = "ansible_project"
    environment = "development"  # if the tags change, would instance get relaunched or it is just the tags
  }
...
resource "aws_iam_instance_profile" "ec2full" {    # is it necessary to put the same name as role here??
  name = "projectec2full-${var.user}"
...
resource "null_resource" "config" {      # it is best practise to do this under null_resource if change is needed, the main instance would not get affected when re-apply-ied.    
